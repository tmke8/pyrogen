...
pub fn indentation_at_offset<'a>(offset: TextSize, locator: &'a Locator) -> Option<&'a str> {
    ...
    let indentation = locator.slice(TextRangeWrapper::new(line_start, offset));
...
pub fn has_leading_content(offset: TextSize, locator: &Locator) -> bool {
    let line_start = locator.line_start(offset);
    let leading = locator.slice(TextRangeWrapper::new(line_start, offset));
...
pub fn has_trailing_content(offset: TextSize, locator: &Locator) -> bool {
    ...
    let trailing = locator.slice(TextRangeWrapper::new(offset, line_end));
...
mod tests {
    ...
    fn trailing_content() -> Result<(), ParseError> {
        let contents = "x = 1";
        let program = ast::Suite::parse(contents, "<filename>")?;
