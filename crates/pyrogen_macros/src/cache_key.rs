...
pub(crate) fn derive_cache_key(item: &DeriveInput) -> syn::Result<TokenStream> {
    ...
    Ok(quote!(
        impl #impl_generics pyrogen_cache::CacheKey for #name #ty_generics #where_clause {
            fn cache_key(&self, key: &mut pyrogen_cache::CacheKeyHasher) {
                use std::hash::Hasher;
                use pyrogen_cache::CacheKey;
                #fields
            }
        }
    ))
